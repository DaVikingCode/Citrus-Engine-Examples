...
package games.hungryhero.com.hsharma.hungryHero.screens {
	...
	import games.hungryhero.Sounds;
	...
	import games.hungryhero.com.hsharma.hungryHero.gameElements.Hero;
	import games.hungryhero.com.hsharma.hungryHero.gameElements.Item;
	import games.hungryhero.com.hsharma.hungryHero.gameElements.Obstacle;
	import games.hungryhero.com.hsharma.hungryHero.gameElements.Particle;
	import games.hungryhero.com.hsharma.hungryHero.objectPools.PoolItem;
	import games.hungryhero.com.hsharma.hungryHero.objectPools.PoolObstacle;
	import games.hungryhero.com.hsharma.hungryHero.objectPools.PoolParticle;
	import games.hungryhero.com.hsharma.hungryHero.ui.GameOverContainer;
	import games.hungryhero.com.hsharma.hungryHero.ui.HUD;
	import games.hungryhero.com.hsharma.hungryHero.ui.PauseButton;
	...
	public class InGame extends StarlingState
	{
		...
		public static var particleMushroom:CitrusSprite;
		...
		private function drawGame():void
		{
			...
			// Draw hero.
			hero = new Hero("hero", {view:new MovieClip(Assets.getAtlas().getTextures("fly_"), 20)});
			add(hero);
		...
		private function foodItemCreate():Item
		{
			...
			foodItem.x = stage.stageWidth + foodItem.view.width * 2;
			add(foodItem);
		...
		private function eatParticleCreate():Particle
		{
			...
			eatParticle.x = stage.stageWidth + eatParticle.view.width * 2;
			add(eatParticle);
		...
		{
			var windParticle:Particle = new Particle("windparticle", {typeParticle:GameConstants.PARTICLE_TYPE_2});
			windParticle.x = stage.stageWidth + windParticle.view.width * 2;
			add(windParticle);
		...
		private function onGameTick(event:Event):void
		{
			...
			if (!gamePaused)
			{
				...
				if (isHardwareRendering)
				{
					((view.getArt(particleCoffee) as StarlingArt).content as PDParticleSystem).emitterX = hero.x + hero.width * 0.5 * 0.5;
					((view.getArt(particleCoffee) as StarlingArt).content as PDParticleSystem).emitterY = hero.y;
					
					((view.getArt(particleMushroom) as StarlingArt).content as PDParticleSystem).emitterX = hero.x + hero.width * 0.5 * 0.5;
					((view.getArt(particleMushroom) as StarlingArt).content as PDParticleSystem).emitterY = hero.y;
				}
				...
				switch(gameState)
				{
					...
					case  GameConstants.GAME_STATE_IDLE:
						...
						if ((-(hero.y - touchY) * 0.2) < 30 && (-(hero.y - touchY) * 0.2) > -30) hero.rotation = -(hero.y - touchY) * 0.2;
						
						// Limit the hero's rotation to < 30.
						if (hero.rotation > 30 ) hero.rotation = 30;
						if (hero.rotation < -30 ) hero.rotation = -30;
					...
					case GameConstants.GAME_STATE_FLYING:
						...
						else
						{
							...
							if (coffee <= 0)
							{
								...
								// Spin the hero.
								if (hero.y > stage.stageHeight * 0.5) hero.rotation -= hitObstacle * 2;
								else hero.rotation += hitObstacle * 2;
							}
		...
		private function createWindForce():void
		{
			...
			windToTrack.view.scaleX = windToTrack.view.scaleY = Math.random() * 0.5 + 0.5;
					...
					else
					{
						...
						if (heroItem_sqDist < 5000)
						{
							...
							else if (itemToTrack.foodItemType == GameConstants.ITEM_TYPE_COFFEE) 
							{
								...
								coffee = 5;
								if (isHardwareRendering) ((view.getArt(particleCoffee) as StarlingArt).content as PDParticleSystem).start(coffee);
							...
							else if (itemToTrack.foodItemType == GameConstants.ITEM_TYPE_MUSHROOM) 
							{
								...
								mushroom = 4;
								if (isHardwareRendering) ((view.getArt(particleMushroom) as StarlingArt).content as PDParticleSystem).start(mushroom);
				...
				for (var i:uint = 0; i < obstaclesToAnimateLength ; i ++)
				{
					...
					if (heroObstacle_sqDist < 5000 && !obstacleToTrack.alreadyHit)
					{
						...
						if (coffee > 0) 
						{
							...
							if (obstacleToTrack.position == "bottom") obstacleToTrack.rotation = 100;
							else obstacleToTrack.rotation = -100;
						...
						{
							if (obstacleToTrack.position == "bottom") obstacleToTrack.rotation = 70;
							else obstacleToTrack.rotation = -70;
		...
		private function animateWindParticles():void
		{
			...
			for(var i:uint = 0;i < windParticlesToAnimateLength;i++)
			{
				...
				if (windToTrack)
				{
					...
					windToTrack.x -= 100 * windToTrack.view.scaleX; 
